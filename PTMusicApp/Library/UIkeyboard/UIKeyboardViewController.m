...
- (void)checkBarButton:(id)textField {
    ...
    if (j==i) {
        [nextBarItem setEnabled: NO];
    }
    else{
        [nextBarItem setEnabled:YES];
    }
}
...
-(void)toolbarButtonTap:(UIButton *)button {
	...
	if (buttonTag == 3) {
        ...
        [self.boardDelegate alttextFieldDidEndEditing:nil];
    }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if(self.isShowButton==100)
        [self.boardDelegate textFieldShouldEnd:nil];
	return YES;
}
...
- (void)addToolbarToKeyboard {
    ...
    keyboardToolbar.isShowButton = self.isShowButton;
    if(self.isShowButton==100){
        [keyboardToolbar removeKeyboard];
    }
    if (self.isShowButton == 200) {
        [keyboardToolbar removeDoneButton];
    }
